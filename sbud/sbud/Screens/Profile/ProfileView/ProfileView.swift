...
private extension ProfileView {
    ...
    var followButtons: some View {
        Button {
            ...
            ZStack {
                if vm.isFollowLoading {
                    ProgressView().tint(vm.followStatus == .notFollowing ? .black : .white)
                } else {
                    HStack(spacing: 6) {
                        if vm.isPending {
                    ...
                    .foregroundColor(followButtonForeground)
                }
            }
            .frame(maxWidth: .infinity)
            .frame(height: 50)
    ...
    private var followButtonBackground: Color {
        switch vm.followStatus {
        case .following:    return Color(white: 0.2)
        case .pending:      return Color(white: 0.25)
        case .notFollowing: return Color("palelime")
        }
    }
    ...
    var statsRow: some View {
        HStack(spacing: 0) {
            Button {
                coordinator.goToFollowerList(userId: vm.userId)
            } label: {
                statItem(value: formatCount(vm.profile?.followersCount ?? 0), label: "FOLLOWERS")
            }
            Rectangle().fill(Color(white: 0.15)).frame(width: 1, height: 28)
            Button {
                coordinator.goToFollowingList(userId: vm.userId)
            } label: {
                statItem(value: formatCount(vm.profile?.followingCount ?? 0), label: "FOLLOWING")
            }
            Rectangle().fill(Color(white: 0.15)).frame(width: 1, height: 28)
